...
Two compose files at the repo root, split by audience:

- **`compose.yaml`** (default, picked up by bare `docker compose up`) — production / self-host. Pulls the gateway and web **images from GHCR** (`ghcr.io/darwinsubramaniam/sharam-ledger/{gateway,web}`). Tag is `${SHARAM_VERSION:-latest}`; pin a release in `.env` for reproducible deploys. No build context, no source checkout required beyond this file + `.env` + volumes.
- **`dev.compose.yaml`** (opt-in via `-f dev.compose.yaml`) — development. Defaults to backing services only (`surrealdb` + `rustfs` + `mailhog`, host ports `127.0.0.1:8000`/`9000`/`1025`+`8025`) so `cargo run -p gateway` and `dx serve --package sharam-ui` on the host get HMR. Adds `gateway` + `web` built from the local Dockerfiles under the `app` profile (`--profile app up -d --build`) — useful for verifying image builds before pushing.

In both cases, gateway/web containers do **not** read `Sharam.toml`; figment falls through to `SHARAM_*` env vars injected by compose.

### File map

- `compose.yaml` — production stack, GHCR images
- `dev.compose.yaml` — dev backing services + optional local-build app profile
...
- `.env.example` — copy to `.env`. Required: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`. Optional: `SHARAM_VERSION` to pin GHCR image tag (defaults to `latest`). Compose interpolation uses `:?` so the build refuses to start without the required keys.
...
- Both compose files pin `surrealdb/surrealdb` and `rustfs/rustfs` to specific tags. The SurrealDB healthcheck shells out to `/surreal is-ready`; if that flag spelling changes between releases, swap for a TCP probe.
- `compose.yaml` floats gateway/web on `latest` by default. For real deploys set `SHARAM_VERSION=0.3.1` (or whatever) in `.env` so a `docker compose pull` doesn't silently roll you forward.
